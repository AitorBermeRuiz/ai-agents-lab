...
class Chat:
    ...
    def system_prompt(self):
        return (f"""
                ...
                Important!! If the user's message is not in English, return a response indicating that you can only understand questions in English.
                """
            )
    ...
    async def _execute_with_tools(self, messages):
        """Ejecuta la llamada a OpenAI manejando tool calls en un bucle (async)."""
        done = False
        while not done:
    ...
    async def chat(self, question, history=None, context=None):
        if history is None:
            history = []
        if context:
    ...
    async def rerun(self, original_reply, message, history, feedback):
